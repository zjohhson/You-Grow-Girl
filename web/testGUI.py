...
def request_handler(request):
    if request['method'] == 'GET':
        ...
        <h1 style="font-family:courier;"text-align:center">Welcome to You Grow Girl!</h1>
        <br>
        <h2 style="font-family:courier;"text-align:center">Pick an action below.</h2>
        <br>
        <head>
        ...
        * {
        ...
        <div class="row">
          <div class="column" style="background-color:darkseagreen;">
            <a href="enterGUI.py" style = "font-size: 20px; font-family: courier;">Enter your plant into database.</a>
          </div>
          <div class="column" style="background-color:darkseagreen;">
            <a href="analyticsGUI.py" style = "font-size: 20px; font-family: courier;">See analytics on your plant.</a>
          </div>
          <div class="column" style="background-color:darkseagreen;">
            <a href="lookupGUI.py" style = "font-size: 20px; font-family: courier;">Look up optimal ranges for a specific plant type.</a>
          </div>
        </div>
            ...
            if plant_name == '':
                ...
                with sqlite3.connect(plant_data_db) as c:
                    ...
                    if check_names(plant_names, plant_name):
                        return r'''<div>{} already exists in database.</div><br><a href="http://608dev-2.net/sandbox/sc/team07/GUI.py">Click here to submit another plant.</a>'''.format(plant_name)
                    else:
                        c.execute("""INSERT INTO plant_data VALUES (?,?,?,?,?,?,?);""", (plant_name, plant_type, 0, 0, 0, 0, now,))
                        return r'''<div>{} the {} entered into database!</div><br><a href="http://608dev-2.net/sandbox/sc/team07/GUI.py">Click here to submit another plant.</a>'''.format(plant_name, plant_type)
