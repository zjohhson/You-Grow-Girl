    ...
    if request['method'] == "GET":
        ...
        with sqlite3.connect(plant_data_db) as c:
            ...
            for i in range(len(light)):
                if light[i] == 0:
                    light[i] = 1
                R1 = R2*(4095/light[i] - 1)
                light[i] = (10**(8.65)/R1)**0.5
            light_figure = figure(x_axis_type="datetime", plot_height = 300, y_range = (0,1000))
            light_figure.line(times, light, legend_label = id + " light (Lux)", line_width = 4, color = 'yellow')
            ...
            return f'''<!DOCTYPE html>
                <html> <script src="https://cdn.bokeh.org/bokeh/release/bokeh-2.3.0.min.js"></script>
                            <body style="background-color:darkseagreen;">
                                {div1}
                    {div2}
		<a href="http://608dev-2.net/sandbox/sc/team07/web/analyticsGUI.py" style = "font-size: 20px; font-family: courier;">Back to previous page.</a>
                            </body>
                                {script1}
                    {script2}
